...
export function useStakingInfo(
  ...
  console.log('cnt-farm start-index', { startIndex, endIndex });
  const { account } = useActiveWeb3React();
...
export function useCntStakingInfo(
  ...
  console.log('cnt-farm start-index', { startIndex, endIndex });
  const { account } = useActiveWeb3React();
  const activeFarms = useDefaultCntFarmList()[chainId];

  console.log('cnt-farms useCntStakingInfo', activeFarms);
  const info = useMemo(
    () =>
      Object.values(activeFarms)
        .filter((x) => !x.ended)
